...
def _missing(field_id: str, notes: str) -> MappedField:
    return MappedField(
        field_id=field_id,
        proposed_value=None,
        confidence=0.0,
...
def _purpose_of_trip(dossier: ApplicantDossier) -> MappedField:
    ...
    return _ready("travel.purpose_of_trip", travel.visa_class)
...
def _security_answer(field_id: str, answer_key: str) -> Resolver:
    def resolve(dossier: ApplicantDossier) -> MappedField:
        ...
        return _ready(field_id, answer, confidence=0.93)

    return resolve


def _family_field(field_id: str, value: str | None, notes: str) -> MappedField:
    if not value:
        return _review(field_id, None, notes, confidence=0.4)
    return _ready(field_id, value, confidence=0.9)


def _resolve_personal_contact(field_id: str, value: str | None) -> MappedField:
    if not value:
        return _missing(field_id, "Personal contact info not provided in dossier.")
    return _ready(field_id, value, confidence=0.85)
...
def _ready_optional(
    ...
    if not value:
        return _review(field_id, None,
                       notes or f"{field_id} is not provided; needs manual input.",
                       confidence=0.4)
    return _ready(field_id, value, confidence=confidence, notes=notes)


def _ready_optional_bool(
    field_id: str, value: bool | None,
) -> MappedField:
    if value is None:
        return _review(field_id, None,
                       f"{field_id} is not provided; needs manual input.",
                       confidence=0.4)
    return _ready(field_id, "YES" if value else "NO", confidence=0.9)


def _family_field_dob(field_id: str, value: str | None, notes: str) -> MappedField:
    if not value:
        return _review(field_id, None, notes, confidence=0.4)
    return _ready(field_id, value, confidence=0.85)
...
def _resolve_previous_employed(dossier: ApplicantDossier) -> MappedField:
    ...
    if e.previous_employer_name:
        return _ready(field_id, "YES", confidence=0.9)
    return _ready(field_id, "NO", confidence=0.85,
...
FIELD_RESOLVERS: list[Resolver] = [
    ...
    lambda dossier: _ready("identity.sex", dossier.identity.sex),
    lambda dossier: _ready(
    ...
    # ---- identity extras ----
    lambda dossier: _ready(
        "identity.other_nationality", "NO", confidence=0.9,
        notes="Default NO for Chinese applicants without other nationality.",
    ),
    lambda dossier: _ready(
        "identity.permanent_resident_other_country", "NO", confidence=0.9,
        notes="Default NO for applicants without foreign permanent residency.",
    ),
    lambda dossier: _ready(
        "identity.national_id_number", "DOES NOT APPLY", confidence=0.9,
        notes="Chinese applicants use passport number, not national ID.",
    ),
    lambda dossier: _ready(
        "identity.us_social_security_number", "DOES NOT APPLY", confidence=0.9,
        notes="Default DOES NOT APPLY unless applicant has US SSN.",
    ),
    lambda dossier: _ready(
        "identity.us_taxpayer_id_number", "DOES NOT APPLY", confidence=0.9,
    ...
    lambda dossier: _ready_optional(
        "employment.current_employer_city", dossier.employment_education.employer_city, confidence=0.85,
    ),
    lambda dossier: _ready_optional(
        "employment.current_employer_state", dossier.employment_education.employer_state, confidence=0.85,
    ),
    lambda dossier: _ready_optional(
        "employment.current_employer_postal_code", dossier.employment_education.employer_postal_code, confidence=0.85,
    ),
    lambda dossier: _ready_optional(
        "employment.current_employer_country", dossier.employment_education.employer_country, confidence=0.88,
    ),
    lambda dossier: _ready_optional(
        "employment.current_employer_phone", dossier.employment_education.employer_phone, confidence=0.85,
    ),
    lambda dossier: _ready_optional(
        "employment.current_employment_start_date", dossier.employment_education.current_employment_start_date, confidence=0.85,
    ),
    lambda dossier: _ready_optional(
        "employment.current_job_duties", dossier.employment_education.current_job_duties, confidence=0.85,
    ),
    lambda dossier: _ready_optional(
        "employment.current_supervisor_surname", dossier.employment_education.current_supervisor_surname, confidence=0.85,
    ),
    lambda dossier: _ready_optional(
        "employment.current_supervisor_given_name", dossier.employment_education.current_supervisor_given_name, confidence=0.85,
    ),
    # ---- previous employment ----
    lambda dossier: _resolve_previous_employed(dossier),
    lambda dossier: _ready_optional(
        "employment.previous_employer_name", dossier.employment_education.previous_employer_name, confidence=0.85,
    ),
    lambda dossier: _ready_optional(
        "employment.previous_employer_city", dossier.employment_education.previous_employer_city, confidence=0.85,
    ),
    lambda dossier: _ready_optional(
        "employment.previous_employer_state", dossier.employment_education.previous_employer_state, confidence=0.85,
    ),
    lambda dossier: _ready_optional(
        "employment.previous_employer_postal_code", dossier.employment_education.previous_employer_postal_code, confidence=0.85,
    ),
    lambda dossier: _ready_optional(
        "employment.previous_employer_country", dossier.employment_education.previous_employer_country, confidence=0.88,
    ),
    lambda dossier: _ready_optional(
        "employment.previous_employer_phone", dossier.employment_education.previous_employer_phone, confidence=0.85,
    ),
    lambda dossier: _ready_optional(
        "employment.previous_job_title", dossier.employment_education.previous_job_title, confidence=0.85,
    ),
    lambda dossier: _ready_optional(
        "employment.previous_supervisor_surname", dossier.employment_education.previous_supervisor_surname, confidence=0.85,
    ),
    lambda dossier: _ready_optional(
        "employment.previous_supervisor_given_name", dossier.employment_education.previous_supervisor_given_name, confidence=0.85,
    ),
    lambda dossier: _ready_optional(
        "employment.previous_employment_start_date", dossier.employment_education.previous_employment_start_date, confidence=0.85,
    ),
    lambda dossier: _ready_optional(
        "employment.previous_employment_end_date", dossier.employment_education.previous_employment_end_date, confidence=0.85,
    ),
    lambda dossier: _ready_optional(
        "employment.previous_job_duties", dossier.employment_education.previous_job_duties, confidence=0.85,
    ),
    # ---- education ----
    lambda dossier: _resolve_other_education(dossier),
    lambda dossier: _ready_optional(
        "employment.school_address_line1", dossier.employment_education.school_address_line1, confidence=0.85,
    ),
    lambda dossier: _ready_optional(
        "employment.school_city", dossier.employment_education.school_city, confidence=0.85,
    ),
    lambda dossier: _ready_optional(
        "employment.major_or_course_of_study", dossier.employment_education.major_or_course_of_study, confidence=0.85,
    ),
    lambda dossier: _ready_optional(
        "employment.school_attendance_start_date", dossier.employment_education.school_attendance_start_date, confidence=0.85,
    ),
    lambda dossier: _ready_optional(
        "employment.school_attendance_end_date", dossier.employment_education.school_attendance_end_date, confidence=0.85,
    ),
    # ---- additional ----
    lambda dossier: _ready_optional(
        "employment.languages", dossier.employment_education.languages, confidence=0.85,
    ),
    lambda dossier: _ready_optional(
        "employment.countries_visited", dossier.employment_education.countries_visited, confidence=0.85,
    ),
    lambda dossier: _ready_optional(
        "employment.clan_or_tribe_name", dossier.employment_education.clan_or_tribe_name, confidence=0.85,
    ),
    lambda dossier: _ready_optional(
        "employment.organization_memberships", dossier.employment_education.organization_memberships, confidence=0.85,
    ),
    lambda dossier: _ready_optional(
        "employment.specialized_skills_description", dossier.employment_education.specialized_skills_description, confidence=0.85,
    ),
    lambda dossier: _ready_optional(
        "employment.military_service_country", dossier.employment_education.military_service_country, confidence=0.85,
    ),
    lambda dossier: _ready_optional(
        "employment.military_branch", dossier.employment_education.military_branch, confidence=0.85,
    ),
    lambda dossier: _ready_optional(
        "employment.military_rank", dossier.employment_education.military_rank, confidence=0.85,
    ),
    lambda dossier: _ready_optional(
        "employment.military_service_start_date", dossier.employment_education.military_service_start_date, confidence=0.85,
    ),
    lambda dossier: _ready_optional(
        "employment.military_service_end_date", dossier.employment_education.military_service_end_date, confidence=0.85,
    ),
    lambda dossier: _ready_optional(
        "employment.insurgent_organization_explanation", dossier.employment_education.insurgent_organization_explanation, confidence=0.85,
    ),
    # ---- family expansions ----
    lambda dossier: _family_field_dob(
        "family.father_date_of_birth", dossier.family_contacts.father_date_of_birth, "Father's date of birth should be confirmed.",
    ),
    lambda dossier: _family_field_dob(
        "family.mother_date_of_birth", dossier.family_contacts.mother_date_of_birth, "Mother's date of birth should be confirmed.",
    ),
    lambda dossier: _family_field_dob(
        "family.spouse_date_of_birth", dossier.family_contacts.spouse_date_of_birth, "Spouse's date of birth should be confirmed if applicable.",
    ),
    lambda dossier: _ready_optional(
        "family.spouse_nationality", dossier.family_contacts.spouse_nationality, confidence=0.85,
    ),
    lambda dossier: _ready_optional(
        "family.spouse_birth_city", dossier.family_contacts.spouse_birth_city, confidence=0.85,
    ),
    lambda dossier: _ready_optional(
        "family.spouse_birth_country", dossier.family_contacts.spouse_birth_country, confidence=0.85,
    ),
    lambda dossier: _ready(
        "family.father_in_us", "NO", confidence=0.9,
        notes="Default NO unless father lives in the US.",
    ),
    lambda dossier: _ready(
        "family.mother_in_us", "NO", confidence=0.9,
        notes="Default NO unless mother lives in the US.",
    ),
    lambda dossier: _ready(
        "family.has_us_immediate_relatives", "NO", confidence=0.9,
        notes="Default NO unless applicant has immediate relatives in the US.",
    ),
    lambda dossier: _ready(
        "family.has_us_other_relatives", "NO", confidence=0.9,
    ...
    # ---- travel companions ----
    lambda dossier: _ready(
        "travel_companions.has_companions", "NO", confidence=0.9,
